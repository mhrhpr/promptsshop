  ...
  return <>
    <section className="hero">
      <div className="shell hero-grid">
        <div className="hero-copy"><div className="kicker"><span className="pulse"/> OMIND / PROMPTSHOP</div><h1>AI برای <em>کار واقعی.</em><br/>مسئله‌ات را بده؛ خروجی بگیر.</h1><p className="hero-sub">از نقش و واحد سازمانی تا مسئله و Workflow، مسیرت را کوتاه می‌کنیم تا سریع‌تر به یک خروجی قابل استفاده برسی.</p><form action="/search" className="hero-search"><input name="q" placeholder="چه کاری باید انجام بدهی؟ مثال: تحلیل ترک خدمت" aria-label="جستجوی workflow"/><button type="submit">پیدا کن <span>↗</span></button></form><div className="hero-meta"><span>۳۰۰۰+ Workflow در نقشه محصول</span><span>۳ Workflow رایگان برای شروع</span><span>Built by OMIND</span></div></div>
        <div className="hero-visual"><div className="hero-orbit orbit-one"/><div className="hero-orbit orbit-two"/><div className="hero-spark spark-a"/><div className="hero-spark spark-b"/><div className="hero-spark spark-c"/><div className="workflow-window glass-card"><div className="window-top"><span className="window-dots">•••</span><span>OMIND COPILOT / LIVE</span><span>AI READY</span></div><div className="window-body"><small>WORKFLOW</small><h3>Employee Turnover Diagnosis</h3><div className="signal-row"><span>INPUT</span><b>Data + Context</b></div><div className="scan-line"/><div className="signal-row"><span>OUTCOME</span><b className="accent-text">۳ driver + ۲ segment + ۵ action</b></div><div className="mini-result"><b>Evidence</b><b>Risk</b><b>Next Action</b></div><Link href="/workflows/hr-turnover" className="mini-button">مشاهده خروجی ↗</Link></div></div></div>
      </div>
    </section>

    <section className="section shell units-section"><div className="section-head"><div><span className="section-index">01</span><div><h2>اول واحدت را پیدا کن</h2><p>ساختار واقعی سازمان؛ کارت‌محور، سریع و قابل فهم.</p></div></div><Link href="/roles">همه واحدها و نقش‌ها ↗</Link></div><div className="unit-grid">{units.map(([n,name,desc,icon]) => <Link href="/roles" className="unit-card" key={name}><span className="unit-no">{n}</span><span className="unit-icon">{icon}</span><h3>{name}</h3><p>{desc}</p><span className="unit-arrow">↗</span></Link>)}</div></section>

    <section className="section section-soft"><div className="shell"><div className="section-head"><div><span className="section-index">02</span><div><h2>Role → Problem → Workflow</h2><p>حداکثر سه حرکت تا کار اصلی.</p></div></div><Link href="/search">جستجو ↗</Link></div><div className="workflow-grid">{free.map(w => <WorkflowCard key={w.id} workflow={w}/>)}</div></div></section>
